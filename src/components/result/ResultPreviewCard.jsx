...
export function ResultPreviewCard({ result }) {
  const statusMeta =
    result.status === 'passed'
      ? {
          ...
          label: 'First-pass review passed',
        }
      : result.status === 'needs_retouch'
        ? {
            className: 'bg-amber-50 text-amber-700',
            label: 'Editor cleanup required',
          }
        : {
  ...
  const metadataRows = [
    ...
    {
      label: 'Detected tone',
      value: result.metadata?.analysis?.background?.backgroundToneLabel || 'Unknown',
    },
  ];
  ...
  return (
    <div className="surface-card overflow-hidden p-5 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className={`rounded-full px-4 py-2 text-sm font-semibold ${statusMeta.className}`}>{statusMeta.label}</div>
      </div>
